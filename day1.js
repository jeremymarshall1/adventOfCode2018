...
she called you down here. "At 500-year intervals into the past, someone has 
...
"The bad news is that we are detecting roughly fifty anomalies throughout time; the 
device will indicate fixed anomalies with stars. The other bad news is that we only 
have one device and you're the best person for the job! Good lu--" She taps a button 
on the device and you suddenly feel like you're falling. To save Christmas, you need 
...
need to repeat its list of frequency changes many times before a duplicate frequency is 
found, and that duplicates might be found while in the middle of processing the list.
...
var inputArray = [];
var frequency = 0;
var frequencyArray = [frequency];
fs = require('fs')
fs.readFile('./input/day1.txt', 'utf8', function (err,data) {
  if (err) {
  ...
  while(loopCheck){
    for (x = 0; x < inputArray.length; x++) {
        ...
        frequency = eval(frequency + inputArray[x]);
